...
async def HelpWatermark(bot, cmd):
	if not await db.is_user_exist(cmd.chat.id):
		...
		await bot.send_message(
			Config.LOG_CHANNEL,
			f"#NEW_USER: \n\nNew User [{cmd.from_user.first_name}](tg://user?id={cmd.chat.id}) started @{Config.BOT_USERNAME} !!"
		)
...
async def SettingsBot(bot, cmd):
	...
	## --- Checks --- ##
	position_tag = None
	watermark_position = await db.get_position(cmd.chat.id)
	...
	watermark_size = await db.get_size(cmd.chat.id)
...
async def VidWatermarkAdder(bot, cmd):
	...
	if cmd.photo or (cmd.document and cmd.document.mime_type.startswith("image/")):
		...
		watermark_path = Config.DOWN_PATH + "/" + str(cmd.chat.id) + "/thumb.jpg"
	...
	editable = await cmd.reply_text("Downloading Video ...", parse_mode=enums.ParseMode.MARKDOWN)
	...
	dl_loc = Config.DOWN_PATH + "/WatermarkAdder/" + str(cmd.chat.id) + "/"
	if not os.path.isdir(dl_loc):
		os.makedirs(dl_loc)
	the_media = None
	user_info = f"**UserID:** #id{cmd.chat.id}\n**Name:** [{cmd.from_user.first_name}](tg://user?id={cmd.chat.id})"
	## --- Done --- ##
	try:
		forwarded_video = await cmd.forward(Config.LOG_CHANNEL)
		logs_msg = await bot.send_message(chat_id=Config.LOG_CHANNEL, text=f"Download Started!\n\n{user_info}", reply_to_message_id=forwarded_video.id, disable_web_page_preview=True, parse_mode=enums.ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ban User", callback_data=f"ban_{cmd.chat.id}")]]))
		await asyncio.sleep(5)
		c_time = time.time()
	...
	output_vid = main_file_name + "_[" + str(cmd.chat.id) + "]_[" + str(time.time()) + "]_[@AbirHasan2005]" + ".mp4"
	progress = Config.DOWN_PATH + "/WatermarkAdder/" + str(cmd.chat.id) + "/progress.txt"
	try:
	...
	await logs_msg.edit("Watermark Added Successfully!\n\nTrying to Upload ...", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ban User", callback_data=f"ban_{cmd.chat.id}")]]))
	width = 100
	height = 100
	duration = 0
	...
	try:
		video_thumbnail = Config.DOWN_PATH + "/WatermarkAdder/" + str(cmd.chat.id) + "/" + str(time.time()) + ".jpg"
	...
	if (int(file_size) > 2097152000) and (Config.ALLOW_UPLOAD_TO_STREAMTAPE is True) and (Config.STREAMTAPE_API_USERNAME != "NoNeed") and (Config.STREAMTAPE_API_PASS != "NoNeed"):
		...
		try:
			async with aiohttp.ClientSession() as session:
				...
				await logs_msg.edit("Successfully Uploaded File to Streamtape!\n\nI am Free Now!", parse_mode=enums.ParseMode.MARKDOWN, disable_web_page_preview=True)
...
async def CancelWatermarkAdder(bot, cmd):
	...
	if not int(cmd.chat.id) == Config.OWNER_ID:
...
async def sts(_, m):
	...
	if int(m.chat.id) == Config.OWNER_ID:
		total_users = await db.total_users_count()
		msg_text += f"\n\n**Total Users in DB:** `{total_users}`"
	await m.reply_text(text=msg_text, parse_mode=enums.ParseMode.MARKDOWN, quote=True)
	...
	if "refreshmeh" in cb_data:
		...
		if Config.UPDATES_CHANNEL:
			...
			try:
				...
				return
		await bot.send_message(chat_id=Config.LOG_CHANNEL, text=f"#SETTINGS_SET: [{cmd.from_user.first_name}](tg://user?id={cmd.chat.id}) Changed Settings!\n\n**User ID:** #id{cmd.chat.id}\n**Data:** `{cb_data}`", parse_mode=enums.ParseMode.MARKDOWN, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ban User", callback_data=f"ban_{cmd.chat.id}")]]))
		new_position = cb_data.split("_", 1)[1]
		if cb_data.startswith("position_"):
			await db.set_position(cmd.chat.id, new_position)
		elif cb_data.startswith("size_"):
			await db.set_size(cmd.chat.id, new_position)
		watermark_position = await db.get_position(cmd.chat.id)
		if watermark_position == "5:main_h-overlay_h":
			position_tag = "Bottom Left"
		elif watermark_position == "main_w-overlay_w-5:main_h-overlay_h-5":
		...
		await db.delete_user(cmd.chat.id)
		await db.add_user(cmd.chat.id)
